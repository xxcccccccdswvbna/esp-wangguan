...
"""
    ...
    # ========== 4. 写入 1, 2, 3 版本 ==========
    # 🔥 修改：在 api 块中增加 send_raw_hex 自定义服务
    base = f"""esphome:
  name: {{name}}
  friendly_name: {{fn}}
esp32:
...
"""
    ...
    # ========== 5. 4 版本 (Pro) ==========
    # 🔥 修改：同样在 api 块中增加 send_raw_hex 自定义服务
...
"""
    ...
    # ========== 4. 写入 CT1, CT2, CT3 ==========
    # 🔥 修改：使用 PROJECT_PREFIX 动态命名，并添加满血蓝牙代理
    base = f"""esphome:
  name: {{name}}
  friendly_name: {{fn}}
esp32:
...
"""
    ...
    print(f"✅ All 4 YAML files ({PROJECT_PREFIX}1~4.yaml) generated successfully with unified Bluetooth Proxy.")
