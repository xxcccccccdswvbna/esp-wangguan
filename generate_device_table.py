...
def generate_all(config_dir: Path, base_dir: Path):
    ...
    # 🔥 核心校验：记录所有使用过的 light 和 fan ID，防止重复
    ...
    for jf in json_files:
        ...
        if "light" in data:
            ...
            if lid:
                if lid in used_light_ids:
                    print(f"❌ FATAL: Duplicate light ID '{lid}' found in {os.path.basename(jf)}!")
                    print(f"   👉 解决办法: 请将 {os.path.basename(jf)} 中的 light.id 修改为唯一值，例如: 'light.{dev_id.split('.')[-1]}'")
                    return
        ...
        if "fan" in data:
            fid = data["fan"].get("id")
            if fid:
                if fid in used_fan_ids:
                    print(f"❌ FATAL: Duplicate fan ID '{fid}' found in {os.path.basename(jf)}!")
                    print(f"   👉 解决办法: 请将 {os.path.basename(jf)} 中的 fan.id 修改为唯一值，例如: 'fan.{dev_id.split('.')[-1]}'")
                    return
                used_fan_ids.add(fid)
    ...
    cpp += [
        ...
        "void DeviceTable::add_action(std::vector<BLEDevice> &d, const std::string &did, const std::string &action, std::vector<std::string> packets) { "
        "for(auto &device:d){if(device.id==did){BLEAction act;act.name=action;act.packets=std::move(packets);device.actions.emplace(action, std::move(act));return;}} "
        "ESP_LOGE(TAG, \"Device ID not found for action: %s\", did.c_str()); }",
        "} // namespace ble_gateway", "} // namespace esphome", ""
    ]
    (base_dir / "components" / "ble_gateway" / "device_table.cpp").write_text("\n".join(cpp))

    # ========== 2. 实体列表 (严格根据 JSON 内容生成) ==========
...
"""
    ...
    print("✅ All 4 YAML files and C++ generated successfully with strict ID validation.")
